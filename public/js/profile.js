...
function setCustomAmount(amount) {
	var selected = Array.from(document.getElementById("amount")).find(
		(e) => e.value === amount
	);
	if (!selected) {
...
function displayCharacter({ ID, Name, Image }) {
	...
	if (findCaracterInList({ ID, Name, Image }, compareInventory)) {
		additionalClass = "owned"
	}
	list.innerHTML += `<li class="list-group-item character ${additionalClass}">
		  <a href="https://anilist.co/character/${ID}"><img src="${Image}" alt="image"></a>
          <p>${ID}</p>
		  <p>${Name}</p>
        </li>`;
}
